#!/usr/bin/env python3
"""
Simulation V9 Strategy - BTC + ETH + XRP
V9 = V8 + Volume Filter (>= 70% moyenne)
...
def simulate_v9(df_15m, df_1h, df_4h, bet_size=100, entry_price=0.525, volume_threshold=0.7):
    """
    V9 Strategy: V8 + Volume Filter
    - UP: RSI(7) < 38 AND Stoch(5) < 30 AND FTFC != BEARISH AND Volume >= 70%
    - DOWN: RSI(7) > 68 AND Stoch(5) > 75 AND FTFC != BULLISH AND Volume >= 70%
    """
    df = df_15m.copy()
    ...
    for i in range(total):
        ...
        if pd.isna(row['rsi']) or pd.isna(row['stoch']) or pd.isna(row['volume_ratio']):
            continue
        ...
        # V9 signals with FTFC + Volume filter
...
def main():
    print("=" * 70)
    print("SIMULATION V9 - BTC + ETH + XRP")
    print("V9 = V8 + Volume Filter (>= 70% moyenne)")
    ...
    for pair in pairs:
        ...
        print("  Simulating V9...")
        trades = simulate_v9(df_15m, df_1h, df_4h)
    ...
    # Monthly breakdown
    print("\n" + "=" * 70)
    print("DETAIL MENSUEL 2024-2025 (V9 FTFC + VOLUME)")
    print("=" * 70)
    ...
    print("\n" + "=" * 70)
    print("COMPARAISON V7 vs V8 vs V9")
    print("=" * 70)
    print(f"\n{'Metrique':<20} {'V7':>12} {'V8 FTFC':>12} {'V9 FTFC+Vol':>12}")
    print("-" * 60)
    print(f"{'Trades':<20} {'63,630':>12} {'29,204':>12} {total_trades:>12,}")
    print(f"{'Trades/jour':<20} {'87':>12} {'40':>12} {total_trades/730:>12.0f}")
    print(f"{'Win Rate':<20} {'55.5%':>12} {'60.4%':>12} {total_wr:>11.1f}%")
    print(f"{'PnL Total':<20} {'$362,524':>12} {'$441,695':>12} ${total_pnl:>11,.0f}")
    print(f"{'PnL/Mois':<20} {'$15,105':>12} {'$18,404':>12} ${total_pnl/total_months:>11,.0f}")
    print(f"{'Mois < $10k':<20} {'5':>12} {'1':>12} {months_under_10k:>12}")
