#!/usr/bin/env python3
"""
Simulation V10 Strategy - BTC + ETH + XRP
Configuration optimale: 67 trades/jour, 0 mois < $10k
...
def get_ftfc_score(df_1h, df_4h, timestamp):
    """
    Calculate FTFC (Flow/Trend/Fractal Confirmation) score
    Based on 1H and 4H timeframe alignment
    """
...
def simulate_v7_dynamic_wide(df_15m, df_1h, df_4h, bet_size=100, entry_price=0.525):
    """
    V10 Strategy

    UP Signal:
      RSI(7) < 42 AND Stoch(5) < 38 AND FTFC_Score > -2.0

    DOWN Signal:
      RSI(7) > 62 AND Stoch(5) > 68 AND FTFC_Score < 2.0
    """
    df = df_15m.copy()
    ...
    for i in range(total):
        ...
        # Get FTFC score
        ftfc_score = get_ftfc_score(df_1h, df_4h, row.name)

        signal = None

        # V10 signals
        if row['rsi'] < RSI_OVERSOLD and row['stoch'] < STOCH_OVERSOLD:
            if ftfc_score > -FTFC_THRESHOLD:  # Not strongly bearish
                signal = 'UP'
        elif row['rsi'] > RSI_OVERBOUGHT and row['stoch'] > STOCH_OVERBOUGHT:
...
def main():
    print("=" * 70)
    print("SIMULATION V10 - BTC + ETH + XRP")
    print("=" * 70)
    print("Configuration optimale:")
    print("  RSI(7): 42/62")
    print("  Stochastic(5): 38/68")
    print("  FTFC Threshold: 2.0")
    print("  $100/trade | Entry: 52.5c")
    print("=" * 70)
    ...
    for pair in pairs:
        ...
        print("  Simulating V10...")
        trades = simulate_v7_dynamic_wide(df_15m, df_1h, df_4h)
    ...
    # Monthly breakdown
    print("\n" + "=" * 70)
    print("DETAIL MENSUEL 2024-2025 (V10)")
    print("=" * 70)
    ...
    for sig in ['UP', 'DOWN']:
        sig_df = all_df[all_df['signal'] == sig]
        if len(sig_df) > 0:
            sig_wr = sig_df['win'].mean() * 100
            sig_pnl = sig_df['pnl'].sum()
            print(f"\n{sig}:")
            print(f"  Trades: {len(sig_df):,}")
            print(f"  Win Rate: {sig_wr:.1f}%")
            print(f"  PnL: ${sig_pnl:,.0f}")

    # FTFC Score distribution
    print("\n" + "=" * 70)
    print("ANALYSE PAR FTFC SCORE")
    print("=" * 70)

    all_df['ftfc_zone'] = pd.cut(all_df['ftfc_score'],
                                  bins=[-4, -2, -1, 1, 2, 4],
                                  labels=['Strong Bear', 'Bear', 'Neutral', 'Bull', 'Strong Bull'])

    for zone in ['Strong Bear', 'Bear', 'Neutral', 'Bull', 'Strong Bull']:
        zone_df = all_df[all_df['ftfc_zone'] == zone]
        if len(zone_df) > 0:
            zone_wr = zone_df['win'].mean() * 100
            zone_pnl = zone_df['pnl'].sum()
            print(f"  {zone}: {len(zone_df)} trades | WR: {zone_wr:.1f}% | PnL: ${zone_pnl:,.0f}")

    # Strategy summary
    print("\n" + "=" * 70)
    print("RESUME STRATEGIE V10")
    print("=" * 70)
    print("""
    REGLES DE TRADING:
    ...
    UP Signal:
      - RSI(7) < 42
      - Stochastic(5) < 38
      - FTFC Score > -2.0 (pas fortement bearish)

    DOWN Signal:
      - RSI(7) > 62
      - Stochastic(5) > 68
      - FTFC Score < 2.0 (pas fortement bullish)
    ...
    # Validation
    print("\n" + "=" * 70)
    print("VALIDATION DES CRITERES")
    print("=" * 70)
    trades_ok = total_trades / 730 >= 60
    pnl_ok = months_under_10k == 0

    print(f"\n  Trades/jour >= 60: {total_trades / 730:.0f} {'OK' if trades_ok else 'FAIL'}")
    print(f"  Mois >= $10k: {total_months - months_under_10k}/{total_months} {'OK' if pnl_ok else 'FAIL'}")

    if trades_ok and pnl_ok:
        print("\n  >>> STRATEGIE VALIDEE <<<")
    else:
