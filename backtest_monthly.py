#!/usr/bin/env python3
"""
Backtest PnL Mois par Mois - Stratégie RSI + Stochastic
========================================================
...
# Combos jour+heure bloqués (dow, hour) - dow: 0=Lundi, 4=Vendredi
BLOCKED_COMBOS = [
    (4, 14),  # Vendredi 14h
    (4, 4),   # Vendredi 4h
    (2, 6),   # Mercredi 6h
...
def run_backtest() -> pd.DataFrame:
    ...
    for symbol in SYMBOLS:
        ...
        # Résultat réel (prochaine bougie)
...
def print_monthly_report(trades_df: pd.DataFrame):
    ...
    print("=" * 80)
    print("              PNL MOIS PAR MOIS - $100/TRADE @ 52.5¢")
    print("=" * 80)
    ...
    for year in [2024, 2025]:
        ...
        for month in range(1, 13):
            ...
            wr_indicator = "✅" if wr >= 55 else "⚠️" if wr >= 52 else "❌"
        ...
        print(f"\n📈 Meilleur: {best_month} (+${monthly_pnl.max():,.0f})")
        print(f"📉 Pire: {worst_month} (+${monthly_pnl.min():,.0f})")

    # Résumé final
    print(f"\n{'=' * 80}")
    print("                         📊 RÉSUMÉ COMPARATIF")
    print("=" * 80)
    print(f"{'Année':<10} {'Trades':>12} {'Win Rate':>12} {'PnL Total':>18} {'PnL/Mois':>18}")
    print("-" * 80)
    ...
    for year in [2024, 2025]:
        ...
        print(f"{year:<10} {trades:>12,} {wr:>11.2f}% {f'+${pnl:,.0f}':>18} {f'+${pnl/months:,.0f}':>18}")
    ...
    print(f"{'TOTAL':<10} {total_trades:>12,} {total_wr:>11.2f}% {f'+${total_pnl:,.0f}':>18} {f'+${total_pnl/total_months:,.0f}':>18}")
    print("=" * 80)
...
if __name__ == "__main__":
    ...
    print(f"   Mise: ${STAKE} @ {ENTRY_PRICE*100:.1f}¢")
    print(f"   Combos bloqués: {len(BLOCKED_COMBOS)}")
    print()
    ...
    print(f"\n✅ {len(trades_df):,} trades trouvés")

    # Afficher le rapport
    print_monthly_report(trades_df)

    # Optionnel: sauvegarder les trades
